...
- **database** – Node-only ALS + shared-DB + Postgres naming + RLS `SET LOCAL` builders + **`resolveTenantDatabaseUrl`** (per-tenant DSN via env). Depends on core. See `docs/INTERNAL/database-scope.md`, `docs/INTERNAL/shared-db-tenant-id.md`, `docs/INTERNAL/schema-per-tenant-postgres.md`, `docs/INTERNAL/postgres-rls-tenant.md`, `docs/INTERNAL/per-tenant-database-url.md`.
