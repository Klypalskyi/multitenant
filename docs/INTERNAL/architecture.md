...
- **database** – Node-only `AsyncLocalStorage` tenant scope (`runWithTenantScope`, `requireTenantScope`) plus shared-DB helpers (`requireTenantKey`, `assignTenantIdForWrite`, `assertRowTenantColumn`) and Postgres schema naming (`schemaNameForTenant`, `requireSchemaNameForCurrentTenant`). Depends on core. See `docs/INTERNAL/database-scope.md`, `docs/INTERNAL/shared-db-tenant-id.md`, `docs/INTERNAL/schema-per-tenant-postgres.md`.
