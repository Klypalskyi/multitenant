...
| `@multitenant/express` | Express – `tenantifyExpress({ registry, environment })` → `req.tenant` |
| `@multitenant/nest` | NestJS – `TenantifyModuleForRoot({ registry, environment })`, `@Tenant()` param decorator |
