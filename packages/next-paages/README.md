...
- `withTenantGSSP(gssp, { registry, environment? })` – wraps `getServerSideProps` and injects `tenant` into props. If the host does not resolve, returns **`{ notFound: true }`** (Next **`notFound()`** — not a **`TenantNotFoundError`**).
