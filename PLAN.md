...
**Last reviewed:** 2026-03-29 — **Marketing site + CI:** **`apps/site`** (**Next.js** + **Fumadocs** — `fumadocs-mdx` / `fumadocs-ui`, MDX under **`content/docs/`**); **Vercel** Git deploy (**Root Directory** `apps/site`, **`vercel.json`**); **npm** via **`.github/workflows/publish-npm.yml`** (see **`docs/RELEASE.md`** §7). Dashboard **Website** row **Shipped**. *(Prior: **Phase 2** **v0.6.19**.)*
...
| Website / landing in repo | **Shipped** | **`apps/site`** — Fumadocs; **first-class MDX** (`content/docs/`: home, getting started, config, packages w/ adapter guides, CLI, errors, examples); **`npm run site:dev`** + **`npm test -w @multitenant/site`**; Vercel **Root Directory** `apps/site` + **`vercel.json`** |
