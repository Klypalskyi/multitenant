...
- **`cachedFetch<T>(url, { locale, ... })`** — build-time request cache to reduce redundant CMS API calls across locales
- **`createTenantCachedFetch<T>(registry, tenantKey, options?)`** — cached fetch pre-bound to a tenant's locale
...
**`@multitenant/next-app/auto`** — **`createTenantMiddlewareFromConfig(json, { environment })`** for a single-file middleware (see [Examples — Next.js (App Router)](/docs/examples/next-app)).
...
<InstallTabs npm="npm install @multitenant/next-pages next react react-dom" />
...
<CliDlxTabs args="init --framework next-pages" />
...
1. During `next build`: check cache → on miss: fetch → store to `.next/.build-cache/`
2. During `next dev` / runtime: pass through to native `fetch()` (no caching)
3. CLI: `multitenant cache --stats` to view entries; `multitenant cache --locale all` to clear
...
Full guide, performance notes, invalidation strategies, and examples: [Build-Time Cache](/docs/guide/build-time-cache).
...
- [Build-Time Cache guide](/docs/guide/build-time-cache) — detailed API & examples
