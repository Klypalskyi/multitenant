...
**`onMissingTenant: 'throw'`** → pass **`TenantNotFoundError`** to **`next(err)`**; pair with a middleware that uses **`isMultitenantError`** (see [Express](/frameworks/express)).
