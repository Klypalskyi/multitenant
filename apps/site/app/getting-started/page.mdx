...
For scope and mental model, read [Why Multitenant?](/why-multitenant) first.
...
On failure you get the same messages as **`validateTenantsConfig`** in code (**`InvalidTenantsConfigError`**). See [Errors](/errors).
...
Optional: **`npx multitenant dev --run-dev`** to let the CLI spawn **`npm run dev`** for you. Full options: [CLI: dev, check, print](/cli/dev-check-print).
...
In **Next.js**, avoid top-level **`await`** in modules **Edge** may load: bootstrap in **`instrumentation.ts`** (Node) or a lazy server-only module — see [Next.js (App Router)](/frameworks/next-app).
