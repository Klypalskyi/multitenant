# Configuration (`tenants.config.json`)
...
Invalid files throw **`InvalidTenantsConfigError`** — same checks as **`npx multitenant check`**. See [Errors](/errors).
...
- **`config`** (optional) — arbitrary defaults merged into every tenant in this market **before** tenant-level **`config`** (Phase 3.3)
...
Always run **`npx multitenant check`** in CI alongside your JSON edits.
